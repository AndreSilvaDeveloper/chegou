...
- [ ] Mudou o texto de chegada ou de retirada? Eles **não são personalizáveis**:
      são cinco versões fixas de cada tipo em
      `src/modules/notificacoes/message-template.ts`, sorteadas a cada envio, e
      mudá-las vale para toda a plataforma. Mantenha as cinco realmente
      diferentes entre si e rode `message-template.spec.ts`.
      `whatsapp/templates.ts` guarda os textos de resposta ao morador.
